...
| SESSION-START-002 | Stale create rollback | A creating bead with pending create metadata stays creating while its configured provider-start lease is active, even after the generic one-minute projection would mark it asleep. Once that lease expires, state and pending-create metadata roll back together exactly once. Never-started pending create can migrate to start-pending while its rollback lease is active. | `cmd/gc/session_reconcile_test.go`; `cmd/gc/session_reconciler_test.go` |
...
| SESSION-RUNTIME-007 | Empty tmux server observation | A reachable tmux server with zero sessions is a successful empty-fleet observation: the state cache primes, departed sessions become non-running immediately, and repeated reads do not refetch or log an outage. A genuinely unreachable server remains an observation failure and preserves last-known-good state for the bounded stale window. | `internal/runtime/tmux/state_cache_empty_server_test.go`; `internal/runtime/tmux/empty_server_liveness_integration_test.go` |
