...
func TestTmuxConformance(t *testing.T) {
	...
	runtimetest.RunProviderTests(t, func(t *testing.T) (runtime.Provider, runtime.Config, string) {
		return NewSeamBackedWithConfig(tmuxConformanceConfig(t)), runtime.Config{
			Command: "sleep 300",
			WorkDir: t.TempDir(),
...
func tmuxConformanceConfig(t *testing.T) Config {
	t.Helper()
	if _, err := exec.LookPath("tmux"); err != nil {
		t.Fatalf("tmux conformance requires the real tmux executable: %v", err)
	}
	t.Logf("isolated tmux socket root=%s socket=%s", os.Getenv("TMUX_TMPDIR"), testSocketName)
	cfg := DefaultConfig()
