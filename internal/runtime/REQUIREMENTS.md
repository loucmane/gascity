...
| RUNTIME-CONTRACT-006 | Tmux interactions preserve provider submit semantics | Pane-backed pending interactions recognize active Claude and Codex approval menus, ignore resolved historical menus, expose a stable request ID, and submit the selected response using the provider's required keystroke sequence. Codex menus require the numeric choice followed by Enter; Claude menus retain their existing single-key behavior. | `internal/runtime/tmux/interaction.go`; `internal/runtime/tmux/interaction_test.go` |
